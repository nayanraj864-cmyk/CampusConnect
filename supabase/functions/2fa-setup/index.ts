...
serve(async (req) => {
    ...
    try {
        ...
        if (action === "generate") {
            ...
            const otpauth = authenticator.keyuri(user.email ?? "user@campusconnect.com", "CampusConnect", secret);
            ...
            return new Response(
                JSON.stringify({ secret, qrCodeDataUrl }),
        ...
        if (action === "verify_setup") {
            ...
            return new Response(
                JSON.stringify({ success: true, message: "2FA successfully enabled" }),
                { headers: { ...corsHeaders, "Content-Type": "application/json" }, status: 200 }
            );
        }
