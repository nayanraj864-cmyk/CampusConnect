    ...
    if (isLoading) {
        return (
            <div className="flex flex-col items-center justify-center h-64 space-y-4">
                ...
                <p className="text-gray-600 dark:text-gray-300">Crunching massive datasets in the background...</p>
            </div>
        );
    }
    ...
    if (error) {
        return (
            <div className="flex flex-col items-center justify-center h-64 space-y-4 text-red-500">
                ...
                <p>{error}</p>
                <Button onClick={retry} variant="outline">Retry Calculation</Button>
            </div>
        );
    }
        ...
        <div className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <Card>
                    ...
                    <CardContent className="text-3xl font-bold">{stats.totalEvents.toLocaleString()}</CardContent>
                </Card>
                <Card>
                ...
                <Card>
                    <CardHeader><CardTitle>Avg. Attendees</CardTitle></CardHeader>
                    <CardContent className="text-3xl font-bold">{Math.round(stats.averageAttendees)}</CardContent>
                </Card>
                <Card>
                    <CardHeader><CardTitle>Max Duration</CardTitle></CardHeader>
                ...
                <CardContent>
                    <ul className="space-y-2">
                        {Object.entries(stats.categoryBreakdown).map(([category, data]) => (
                            <li key={category} className="flex justify-between items-center p-2 bg-gray-50 dark:bg-gray-800 rounded">
                                <span className="font-medium capitalize">{category}</span>
                                <span className="text-sm text-gray-600 dark:text-gray-400">
                                    {data.count} events | ${data.revenue.toLocaleString(undefined, { maximumFractionDigits: 0 })}
                                </span>
                            </li>
                        ))}
                    </ul>
